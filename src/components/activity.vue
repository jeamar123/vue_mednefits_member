	...
	var activity = {
		data() {
			return {
				...
				end_month : moment( ).endOf('month').format('YYYY-MM-DD'),
			}
		},
		created() {
			this.getDetails();
			this.setFirstEndDate(1,2);
		...
		methods: {
			...
			setFirstEndDate( firstMonth, lastMonth ){
				this.start_month = moment( firstMonth + " " + this.current_year,'MM YYYY' ).startOf('month').format('YYYY-MM-DD');
				this.end_month   = moment( lastMonth + " " + this.current_year,'MM YYYY' ).endOf('month').format('YYYY-MM-DD');
			},
